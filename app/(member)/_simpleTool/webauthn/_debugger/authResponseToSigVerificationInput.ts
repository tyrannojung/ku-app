...
export const authResponseToSigVerificationInput = (
  ...
  console.log(parseAuthData(authDataBuffer));
  ...
  console.log("🚀 ~ authResponse.signature:", authResponse.signature);
  ...
  const publicKeyCoordinates = [
    "0x" +
      base64url.toBuffer(parsedCredentialPublicKey?.x || "").toString("hex"),
    "0x" +
      base64url.toBuffer(parsedCredentialPublicKey?.y || "").toString("hex"),
  ];
