...
import { testGenerateWebAuthnLoginOptions } from "./webauthn/testwebauthn";
...
import { testBundlerCall2 } from "./_testTool/testbundlerTool2";
...
export default function Test() {
  ...
  const handleSignUpClick = async () => {
    ...
    try {
      ...
      let new_push1 = `0x${base64url
        .toBuffer(passkey.response.authenticatorData!)
        .toString("hex")}`;
      let new_push2 = `0x${base64url
      ...
      // messages.push(
      //   `const rs = ["${ecVerifyInputs.signature[0]}", "${ecVerifyInputs.signature[1]}"]`
      // );
      // messages.push(
      //   `const Q = ["${ecVerifyInputs.publicKeyCoordinates[0]}", "${ecVerifyInputs.publicKeyCoordinates[1]}"]`
      // );
  ...
  const handleSignInClick = async () => {
    ...
    try {
      ...
      console.log(
        "challengeOffsetRegex, challengePrefix",
        challengeOffsetRegex,
        challengePrefix
      );
      ...
      messages.push(
        `const Q = ["${response.user.pubkCoordinates[0]}", "${response.user.pubkCoordinates[1]}"]`
      );
      ...
      const bundlerSendResult: boolean = await testBundlerSend(
        userOperation,
        response.user
      );
  ...
  const secp256k1TestClick = async () => {
    await testBundlerCall2();
  };

  return (
    <div>
      <h1 style={{ textAlign: "center", color: "#333", fontSize: "24px" }}>
        Welcome to the WebAuthn Test!
      </h1>
      <div className={styles.formStyle}>
        ...
        <div className={styles.buttonContainerStyle}>
          <button className={styles.buttonStyle} onClick={handleSignUpClick}>
            sign up
          </button>
          <button className={styles.buttonStyle} onClick={handleSignInClick}>
            sign in
          </button>
          <button className={styles.buttonStyle} onClick={secp256k1TestClick}>
            secp256k1 test
          </button>
        </div>
      </div>
